...
EMM <- EMM %>%
  mutate(
    ...
    Level = factor(Level,
                   levels = c("Light-roasted Arabica", "Dark-roasted Robusta",        # Coffee type
...
ggsave("Hedonic_plot_rev2.png", plot, width = 10, height = 6, dpi = 600)
```
...
plot <- ggplot(EMM2, aes(x = Level, y = Mean, color = Country, group = Country)) +
  geom_point(size = 3, position = position_dodge(0.3)) +
  geom_line(position = position_dodge(0.3), size = 1) +
  facet_wrap(~ Attribute, scales = "free_x", nrow = 1) +
  labs(
...
ggsave("Hedonic_plot_rev3.png", plot, width = 10, height = 6, dpi = 600)
```
